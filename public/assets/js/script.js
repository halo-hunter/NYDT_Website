$(document).ready(function(){
    ...
    if (current_page_url.includes('clients/edit')) {
        ...
        create_a_rider_profile_button.addEventListener('click', function (e) {
            ...
            const add_rider_email_input = document.getElementById('add_rider_email_input_id')
            const add_rider_phone_input = document.getElementById('add_rider_phone_input_id')

            const add_rider_close_button = document.getElementById('add_rider_close_button_id')


            function check_firstname_lastname_input_values() {
                ...
                if (add_rider_firstname_input.value != '' && add_rider_lastname_input.value != '') {
            ...
            document.querySelectorAll('#add_rider_firstname_input_id, #add_rider_lastname_input_id')
                .forEach(input => {
                    input.addEventListener('keyup', check_firstname_lastname_input_values);
                });
            ...
            add_rider_button.addEventListener('click', function (e) {
                ...
                if (add_rider_firstname_input.checkValidity() && add_rider_lastname_input.checkValidity()) {
                    ...
                    function postWithData(url, firstname, lastname, email, phone, client_id) {
                        const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
                        const data = {
                            firstname: firstname,
                            lastname: lastname,
                    ...
                    postWithData(add_rider_url, add_rider_firstname_input.value, add_rider_lastname_input.value, add_rider_email_input.value, add_rider_phone_input.value, create_a_rider_modal_client_id.value)
                        .then(responseData => {
                            ...
                            if (result == 1) {
                                ...
                                add_rider_firstname_input.value = ''
                                add_rider_lastname_input.value = ''
                                add_rider_email_input.value = ''
                                add_rider_phone_input.value = ''
        ...
        add_rider_close_button.addEventListener('click', function () {
            ...
            const add_rider_firstname_input = document.getElementById('add_rider_firstname_input_id')
            const add_rider_lastname_input = document.getElementById('add_rider_lastname_input_id')
            const add_rider_email_input = document.getElementById('add_rider_email_input')
            const add_rider_phone_input = document.getElementById('add_rider_phone_input')
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('click', function (event) {
        if (event.target.matches('.add-remove-btn')) {
            const action = event.target.dataset.action;

            const dateContainer = document.querySelector('.due-date-container');
            const infoContainer = document.querySelector('.due-date-additional-info-container');

            const dateGroupsCount = dateContainer.querySelectorAll('.due-date-group').length;

            if (action === 'add' && dateGroupsCount < max_fields) {
                // Clone date input
                const dateGroup = dateContainer.querySelector('.due-date-group');
                const dateClone = dateGroup.cloneNode(true);
                dateClone.querySelector('.due-date-input').value = '';

                // Clone additional info input
                const infoGroup = infoContainer.querySelector('.due-date-additional-info-group');
                const infoClone = infoGroup.cloneNode(true);
                infoClone.querySelector('.due-date-additional-info-input').value = '';

                // Update button
                const button = infoClone.querySelector('.add-remove-btn');
                button.textContent = '-';
                button.dataset.action = 'remove';
                button.classList.remove('btn-primary');
                button.classList.add('btn-danger');

                dateContainer.appendChild(dateClone);
                infoContainer.appendChild(infoClone);

                if (dateGroupsCount + 1 === max_fields) {
                    event.target.disabled = true;
                }
            }

            if (action === 'remove') {
                const nodes = [...infoContainer.querySelectorAll('.due-date-additional-info-group')];
                const removeIndex = nodes.indexOf(event.target.closest('.due-date-additional-info-group'));

                dateContainer.querySelectorAll('.due-date-group')[removeIndex].remove();
                infoContainer.querySelectorAll('.due-date-additional-info-group')[removeIndex].remove();

                if (dateContainer.querySelectorAll('.due-date-group').length < max_fields) {
                    document.querySelector('.add-remove-btn[data-action="add"]').disabled = false;
                }
            }
        }
    });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('click', function (event) {
        if (event.target.matches('.add-remove-btn')) {
            ...
            if (action === 'add' && dateGroupsCount < max_fields) {
                // Clone the entire due-date-group
                const dateGroup = dateContainer.querySelector('.due-date-group');
                const clone = dateGroup.cloneNode(true);

                // Reset input values in the cloned group
                clone.querySelector('.due-date-input').value = '';
                clone.querySelector('.due-date-additional-info-input').value = '';

                // Update the add/remove button in the cloned group
                const button = clone.querySelector('.add-remove-btn');
                button.textContent = '-';
                button.dataset.action = 'remove';
                button.classList.remove('btn-primary');
                button.classList.add('btn-danger');
                ...
                // Disable the add button if maximum fields are reached
                if (dateGroupsCount + 1 === max_fields) {
                    event.target.disabled = true;
                }
            }
            ...
            if (action === 'remove') {
                const group = event.target.closest('.due-date-group');
                group.remove();

                // Re-enable add button if fewer than max fields exist after removing
                if (dateContainer.querySelectorAll('.due-date-group').length < max_fields) {
                    const addButton = document.querySelector('.add-remove-btn[data-action="add"]');
...
//
//     // Function to fetch and show toast
//     function checkNotification() {
//         fetch('/notifications/listen')
//             .then(response => response.text())
//             .then(data => {
//
//                 // Convert fetched data to number
//                 const newCount = parseInt(data, 10); // new notification count from the response
//
//                 // Retrieve stored values from localStorage
//                 const storedCount = parseInt(localStorage.getItem('notification_count'), 10);
//                 let shown = parseInt(localStorage.getItem('shown'), 10);
//
//                 // If there's no stored count or the value has changed, update storage and reset shown flag
//                 if (isNaN(storedCount) || newCount !== storedCount) {
//                     localStorage.setItem('notification_count', newCount);
//                     localStorage.setItem('shown', '0');
//                     shown = 0;
//                 }
//
//                 // Show toast if there's a notification and it hasn't been shown yet for this count
//                 if (newCount > 0 && shown === 0) {
//                     toastBootstrap.show();
//                     localStorage.setItem('shown', '1');
//                 }
//
//                 console.log("shown:", localStorage.getItem('shown'));
//                 console.log("notification_count:", localStorage.getItem('notification_count'));
...
//     const closeButton = document.querySelector('#liveToast .btn-close');
//
