...
class PortalCustomerUploadDocuments extends Controller
{
    public function show(Request $request, $case_id) {
        if ($request->isMethod('get')) {
            $data = [
                'case_id' => $case_id,
                'requested_documents' => DB::table('list_of_requested_documents_sent_to_the_client_version_twos')->where('case_id', $case_id)->where('is_uploaded', 0)->get(),
                'uploaded_documents' => DB::table('list_of_requested_documents_sent_to_the_client_version_twos')->where('case_id', $case_id)->where('is_uploaded', 1)->get()
            ];
            ...
            foreach ($request->files as $file) {
                foreach ($file as $k => $v) {
                    $fileName = time().'.'.$request->file('files.' . $k)->extension();
                    $request->file('files.' . $k)->move(public_path('files/requested_documents/'), $fileName);
                    UploadDocumentVersionTwo::insert([
                        'user_id' => 0,
                        'case_id' => $case_id,
                        'name' => $fileName,
                        'description' => str_replace('_', ' ', $k),
                        'requested_document_id' => DB::table('list_of_requested_documents_sent_to_the_client_version_twos')->where('case_id', $case_id)->first()->id,
            ...
            $mail_data = [
                'case_id' => $case_id,
                'customer_firstname_lastname' => Customers::where('id', $case_id)->first()->firstname . ' ' . Customers::where('id', $case_id)->first()->lastname,
                'requested_documents' => DB::table('list_of_requested_documents_sent_to_the_client')->where('customer_id', $case_id)->get()
            ];
            ...
            DB::table('list_of_requested_documents_sent_to_the_client_version_twos')->where('case_id', $case_id)->update([
                'is_uploaded' => true
            ]);
