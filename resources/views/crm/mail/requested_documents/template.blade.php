...
<div style="width: 95%; display: table; margin: 0 auto">
    ...
    <div style="width: 100%; height: auto; margin-top: -16px; margin-left:15px; margin-right: 15px; background-color: #f8f9fa; color: #000000; ">
        ...
        <p style="text-align: left; padding-top: 10px; padding-left: 15px;">Requested document list:</p>
        ...
        <div style="text-align: left; padding-bottom: 15px; padding-left: 15px;">
            <ul>
                @foreach($mail_data['document_types'] as $document_type)
                    <li style="text-transform: capitalize">{{ $document_type }}</li>
                @endforeach
            </ul>
            <div style="margin-bottom: 15px!important;">
                Login to your account here: <a href="{{ route('portal->login->show') }}" target="_blank">{{ route('portal->login->show') }}</a>
            </div>
        </div>
